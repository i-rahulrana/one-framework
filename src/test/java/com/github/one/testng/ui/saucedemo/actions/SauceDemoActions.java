...
public class SauceDemoActions {
    ...
    public void verifyAddToCart () {
        if (this.platformType == PlatformType.WEB) {
            ClickableActions.withMouse (HomePage.homePage ()
                    .getAddToCartButton ())
                .click ();
        } else {
            FingerActions.withFinger (HomePage.homePage ()
                    .getAddToCartDragHandle ())
                .dragTo (HomePage.homePage ()
                    .getCartDropZone ());
        }
    ...
    public void verifyCheckoutStep1 () {
        ClickableActions.withMouse (CartPage.cartPage ()
                .getCheckout ())
            .click ();
        if (this.platformType == PlatformType.WEB) {
        ...
        TextBoxActions.onTextBox (CheckoutPage.checkoutPage ()
                .getZipCode ())
            .enterText ("12345");
        ClickableActions.withMouse (CheckoutPage.checkoutPage ()
                .getContinueButton ())
            .click ();
    }
    ...
    public void verifyCheckoutStep2 () {
        if (this.platformType != PlatformType.WEB) {
            FingerActions.withFinger (CheckoutPage.checkoutPage ()
                    .getFinish ())
                .swipeTill (SwipeDirection.UP);
        }
        ClickableActions.withMouse (CheckoutPage.checkoutPage ()
                .getFinish ())
            .click ();
    ...
    public void verifyLogin (final String userName, final String password) {
        ...
        ClickableActions.withMouse (LoginPage.loginPage ()
                .getLoginButton ())
            .click ();
        verifyLoggedIn ();
    }
    ...
    private void verifyProductCartPage () {
        ...
        ElementActions.onElement (CartPage.cartPage ()
                .getCheckout ())
            .verifyIsDisplayed ()
            .isTrue ();
    }
    ...
    private void verifyProductDetailPage () {
        ClickableActions.withMouse (HomePage.homePage ()
                .productItem ("Sauce Labs Backpack"))
            .click ();
        if (this.platformType == PlatformType.WEB) {
            NavigateActions.navigate ()
                .verifyUrl ()
